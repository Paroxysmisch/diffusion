...
def test_vae_coco_reconstruction(checkpoint_path, coco_root, coco_ann_file, output_path="vae_coco_test.png"):
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=4, shuffle=True, collate_fn=coco_collate_fn)
    ...
    comparison = torch.cat([denormalize(real_images), denormalize(reconstruction)], dim=0)
